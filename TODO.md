...
- [x] **"빠른 반복 + 더 많은 후보" 포지셔닝** — 사실 메커니즘 자체(최대 5개 후보, 실시간 스왑)는 이미 구현돼 있었음(스키마 max=5, PRO는 30분 주기까지). 실제로 빠져있던 건 이 차별화를 알리는 것.
  - 랜딩 페이지에 "Doesn't YouTube already do this?" 비교 섹션 추가 — YouTube Test and Compare(최대 3개, 2주+, 수동 오버라이드 없음, 채널 1개씩) vs ThumbnailFlow(최대 5개, 30분~, 강제 스왑/즉시 승자 확정, 멀티채널 대시보드)를 사실 기반으로 정직하게 비교. YouTube 기능 자체를 깎아내리지 않고 "왜 다른 선택지가 필요한지"를 설명하는 톤으로 작성.
...
- [x] 최종 승자 뱃지는 Analytics CTR 기준으로 (VPH는 진행 중 보조 지표)
  ...
  - ✅ **검증 완료 - 예상대로 동작 불가능함이 확인되어 기능 자체를 제거함.** 실제 라이브 채널의 refresh_token으로 YouTube Analytics API를 직접 호출해본 결과, 처음엔 프로젝트에서 Analytics API 자체가 비활성화 상태(403 accessNotConfigured)였고, 콘솔에서 활성화한 뒤 재호출하니 `400 Unknown identifier (impressions) given in field parameters.metrics.`가 그대로 재현됨 — 위에서 우려했던 대로 `impressions`/`impressionsClickThroughRate`는 공개 API에 존재하지 않는 지표였음(YouTube Studio 내부 전용). 이에 따라 `collect_daily_analytics`(scheduler.py), `get_daily_video_analytics`/`get_youtube_analytics_client`(youtube_api.py), `DailyAnalytics` 모델·테이블, `/api/tests`·`/api/history`의 `daily_analytics` 필드, 대시보드의 "Analytics CTR" 표시, OAuth 스코프의 `yt-analytics.readonly`를 전부 제거함. 라이브 DB의 `daily_analytics` 테이블도 확인 후(0행) DROP. 승자 판정은 원래도 VPH 기준이라 이 제거로 영향 없음 — 오히려 작동 안 하는 기능 때문에 지고 있던 "민감한 스코프" 재검증 부담이 사라짐.
